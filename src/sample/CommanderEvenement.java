...
public class CommanderEvenement implements Initializable
{
    ...
    public void Commander(MouseEvent mouseEvent)
    {
        ...
        Evenement evenet = new Evenement(sample.Evenement.client, sample.Evenement.dateconsommation,LocalTime.now(), sample.Evenement.Nombre_de_personne,menu,
                sample.Evenement.thematique, sample.Evenement.type);
        if (Main.Esi.DateEvenArr.contains(evenet.getDate_Consommation()) == false)
        {
            Main.Esi.LesCommandesAtt.add(evenet);//on ajoute la commande a la liste des commandes en attantes.
            Main.Esi.DateEvenArr.add(evenet.getDate_Consommation());
            Alert alert = new Alert(Alert.AlertType.INFORMATION);
            alert.setTitle("Evenement Programme");
            alert.setHeaderText("L'evenement a ete programme avec succes");
            alert.show();
            Parent window1;
            try {
        ...
        else
        {
            ...
            alert.setHeaderText("Le restaurant est deja programme ce jour ci,Veillez choisir une autre date.");
            alert.show();
            Parent window1;
            try {
                window1 = FXMLLoader.load(getClass().getResource("Client.fxml"));
